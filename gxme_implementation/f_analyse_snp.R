# SNP-WISE ANALYSIS FUNCTION 

# snp_id <- snp_id_chunks[[1]][1]
...
analyse_snp <- function(
  ...
  log_filename <- file.path(
    ...
    , paste0(
      # Date + time
      format( Sys.time(), "%Y-%m-%d_%H%M%OS4" ), "_"
      # , Sys.getpid(), "_"
      # Chunk number
      , "c", chunk_number, "_"
  ...
  # If process-specific temporary directory already exists, delete files left
  ...
  if( dir.exists( process_dir ) ){
    ...
    # Create process-specific temporary directory if it doesn't already exist:
  } else{
  ...
  # # ADD AN EXTRA COLUMN WITH UNBALANCED STRATA FOR TESTING PURPOSES:
  # set.seed(123)
  # sls_strata_dt <- sls_strata_dt %>%
  #   mutate( SL999 = sample( x = 1:3
  #                           , size = nrow(sls_strata_dt)
  #                           , prob = c(0.495, 0.495, 0.01)
  #                           , replace = TRUE )
  #   )
  # sls_snp_id <- c(sls_snp_id, "SL999")
  # THIS FAKE SL WILL CAUSE HAPLINSTRAT TO RETURN A LIST WHERE THE ELEMENT
  # BELONGING TO STRATA ` 3` IS AN "TRY-ERROR" OBJECT
