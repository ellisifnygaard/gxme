# Matrix level summarising function - MAX
...
summarising_function <- function( mat, sl_id ){
  ...
  summarised_mat_vector <- apply(mat, MARGIN = 2, base::max
  ...
  # Test if the vector of column maximums was correctly paired with columns in
  # mat. Pick 5 random columns to check:
  for( k  in sample(1:ncol(mat), 5, replace = FALSE) ){
    stopifnot( 
      max( mat[ -nrow(mat), k], na.rm = TRUE ) == mat[ nrow(mat), k]
    )
    # The maximum value among the rows in column k in mat,
    # should be equal to the column max found in the newly added bottom row
  }
