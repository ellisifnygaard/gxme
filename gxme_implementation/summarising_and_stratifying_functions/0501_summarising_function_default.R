# Matrix level summarising function - DEFAULT (i.e. mean)
...
# mat <- test_2_with_nas
# sl_id <- sl
# summarising_function( mat = sl_probe_matrix, sl_id = sl )

summarising_function <- function( mat, sl_id ){
  
  summarised_mat_vector <- colMeans(mat, na.rm = TRUE) # ignore NAs
  ...
  for( k  in sample(1:ncol(mat), 5, replace = FALSE) ){
    # print(k)
    numerator_mean <- sum( mat[ -nrow(mat), k], na.rm = TRUE ) 
    denominator_mean <- if( any(is.na(mat[ -nrow(mat), k])) ){
      length( mat[ -nrow(mat), k] ) - sum( is.na(mat[ -nrow(mat), k]) )
      } else { length( mat[ -nrow(mat), k] ) }
    
    stopifnot( numerator_mean / denominator_mean == mat[ nrow(mat), k] )
  ...
  return( mat[ nrow(mat), , drop = FALSE] )
}
